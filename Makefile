...
help:  ## Show this help
	@powershell -Command "Get-Content Makefile | Select-String '^[a-zA-Z_-]+:.*?## .*$$' | ForEach-Object { $$match = [regex]::Match($$_, '^([a-zA-Z_-]+):.*?## (.*)$$'); Write-Host ('{0,-20} {1}' -f $$match.Groups[1].Value, $$match.Groups[2].Value) }"
...
status:  ## Show project status
	...
	@powershell -Command "$$srcFiles = (Get-ChildItem src -Recurse -Filter '*.py').Count; Write-Host \"  📁 Source files: $$srcFiles\""
	@powershell -Command "$$testFiles = (Get-ChildItem tests -Recurse -Filter '*.py').Count; Write-Host \"  🧪 Test files: $$testFiles\""
	@powershell -Command "$$docFiles = (Get-ChildItem docs -Recurse -Filter '*.md').Count; Write-Host \"  📚 Documentation: $$docFiles\""
