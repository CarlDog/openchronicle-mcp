# 📜 License

## OpenChronicle Core Engine  
Licensed under the GNU Affero General Public License v3.0  

© 2024–2025 CarlDog / OpenChronicle  

---

This program is free software: you can redistribute it and/or modify it under the terms of the GNU Affero General Public License as published by the Free Software Foundation, version 3.

You are free to:
- Use, study, and modify this software
- Share it publicly
- Deploy it in web-based or networked applications

However:
- If you modify this software and make it available to others (e.g., via a website, cloud platform, or SaaS), you must also publish your modified source code under the same license.
- There is no warranty, express or implied. For details, see the full license text:  
  🔗 https://www.gnu.org/licenses/agpl-3.0.html

---

## 📛 Content Responsibility Disclaimer (Supplemental)

OpenChronicle is a local-first storytelling engine. This software enables the creation of user-generated content including fictional narratives, characters, and media.

**The OpenChronicle development team, contributors, and maintainers are not responsible for the content created using this tool**, including (but not limited to):
- NSFW, explicit, or psychologically intense material
- Fanfiction or derivative works based on existing IP (e.g. movies, books, games)
- Content that violates local or international laws

### You, the user, are solely responsible for:
- The stories you create
- How and where you share or export them
- Compliance with applicable laws and intellectual property rights

**OpenChronicle does not host, monitor, or moderate user-generated content.**

---

## 🧾 Note on Story Content Licensing

Story content, world files, and scene data included in this repository (e.g., `/storage/storypacks/`, `/storage/temp/demo/`) may be licensed separately under Creative Commons.

See `LICENSE-content.md` for details.