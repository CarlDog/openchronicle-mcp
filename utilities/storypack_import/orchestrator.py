...
from .interfaces import (
    ImportContext, ImportResult, ContentFile,
    IContentParser, IMetadataExtractor, IStructureAnalyzer,
    IAIProcessor, IContentClassifier, IValidationEngine,
    IStorypackBuilder, ITemplateEngine, IOutputFormatter
)
...
class StorypackOrchestrator:
    ...
    def __init__(self, 
                 content_parser: Optional[IContentParser] = None,
                 metadata_extractor: Optional[IMetadataExtractor] = None,
                 structure_analyzer: Optional[IStructureAnalyzer] = None,
                 ai_processor: Optional[IAIProcessor] = None,
                 content_classifier: Optional[IContentClassifier] = None,
                 validation_engine: Optional[IValidationEngine] = None,
                 storypack_builder: Optional[IStorypackBuilder] = None,
        ...
        log_system_event("storypack_orchestrator", "Orchestrator initialized", {
            "components_loaded": 9,
    ...
    async def initialize(self, templates_dir: Optional[Path] = None) -> bool:
        """
        ...
        log_system_event("storypack_orchestrator", "Initialization completed", {
    ...
    async def import_storypack(self, source_path: Path, storypack_name: str, 
                              target_dir: Path, import_mode: str = "basic") -> ImportResult:
        """
        ...
        try:
            ...
            is_ready, validation_issues = self._validation_engine.validate_import_readiness(context)
            ...
            is_valid, structure_issues = self._validation_engine.validate_storypack_structure(storypack_path)
            ...
            if result.success:
                ...
                log_error(f"✗ Storypack import completed with issues")
            
        except Exception as e:
            log_error(f"Storypack import failed: {e}")
            result.errors.append(f"Import process failed: {str(e)}")
            result.success = False
            result.processing_time = time.time() - start_time
            result.created_at = datetime.now().isoformat()
        
        # Log final result
        log_system_event("storypack_orchestrator", "Import completed", {
    ...
    async def _process_content_files(self, discovered_files: Dict[str, List[ContentFile]], 
                ...
                try:
                    ...
                    if context.ai_available:
                        try:
                            ...
                            log_warning(f"AI analysis failed for {content_file.path}: {e}")
    ...
    def _generate_content_summary(self, processed_content: Dict[str, Any], 
