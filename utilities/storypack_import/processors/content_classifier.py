...
class ContentClassifier(IContentClassifier):
    ...
    def __init__(self):
        ...
        self.classification_patterns = {
            'character_profile': {
                'keywords': ['age:', 'personality:', 'appearance:', 'background:', 'occupation:', 'skills:'],
                'structures': [r'^\s*\*\*\s*(name|age|personality|appearance)\s*:\*\*', r'^#+\s+(personality|background|appearance)'],
                'weight': 0.8
            },
            'location_description': {
                'keywords': ['geography:', 'climate:', 'population:', 'description:', 'notable features:', 'history:'],
                'structures': [r'^\s*\*\*\s*(location|geography|climate|population)\s*:\*\*', r'^#+\s+(geography|climate|description)'],
                'weight': 0.8
            },
            'narrative_scene': {
                'keywords': ['"', 'said', 'walked', 'looked', 'felt', 'thought'],
                'structures': [r'"[^"]*"', r'\b(he|she|they)\s+(said|walked|looked|felt)'],
                'weight': 0.7
            },
            'lore_document': {
                'keywords': ['history:', 'legend:', 'mythology:', 'ancient', 'tradition:', 'origin:'],
                'structures': [r'^#+\s+(history|legend|mythology|origin)', r'long ago', r'in the beginning'],
                'weight': 0.8
            },
            'dialogue_heavy': {
                'keywords': ['"', "'", 'said', 'asked', 'replied', 'whispered', 'shouted'],
    ...
    def _calculate_classification_score(self, content: str, content_lower: str, 
    ...
    def _determine_structure_type(self, structure_analysis: Dict[str, Any]) -> str:
        ...
        elif structure_analysis['header_count'] >= 3:
            return 'header_structured'
        elif structure_analysis['list_items'] >= 5:
            return 'list_structured'
        elif structure_analysis['dialogue_lines'] >= 3:
            return 'dialogue_structured'
        elif structure_analysis['paragraph_count'] >= 3:
    ...
    def _determine_formatting_style(self, content: str) -> str:
        ...
        markdown_matches = sum(1 for pattern in markdown_indicators 
        ...
        if re.search(r'<[^>]+>', content):
            return 'html'
        elif re.search(r'^\s*\w+\s*:\s*.+$', content, re.MULTILINE):
