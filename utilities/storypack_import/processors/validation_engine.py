...
from ..interfaces import IValidationEngine, ImportContext
...
class ValidationEngine(IValidationEngine):
    ...
    def validate_content_format(self, content: str, expected_type: str) -> Tuple[bool, List[str]]:
        ...
        if long_lines:
            issues.append(f"Lines too long (lines {long_lines[:5]}): maximum {self.content_rules['max_line_length']} characters")
        ...
        if is_valid:
            log_system_event("validation_engine", "Content validation passed", {
                "content_length": len(content),
                "expected_type": expected_type,
                "line_count": len(lines)
            })
        else:
            log_system_event("validation_engine", "Content validation failed", {
                "content_length": len(content),
                "expected_type": expected_type,
    ...
    def validate_import_readiness(self, context: ImportContext) -> Tuple[bool, List[str]]:
        ...
        log_system_event("validation_engine", "Import readiness validation", {
            "is_ready": is_ready,
    ...
    def validate_storypack_structure(self, storypack_path: Path) -> Tuple[bool, List[str]]:
        ...
        log_system_event("validation_engine", "Storypack structure validation", {
            "is_valid": is_valid,
