...
class AIProcessor(IAIProcessor):
    ...
    async def initialize(self) -> bool:
        ...
        try:
            ...
            if success:
                log_system_event("ai_processor", "AI capabilities initialized", {
    ...
    async def analyze_content(self, content: str, file_path: Path, context: ImportContext) -> Dict[str, Any]:
        ...
        try:
            ...
            log_system_event("ai_processor", "Content analysis completed", {
        ...
        try:
            ...
            **Appearance:** Tall, brown hair, wearing silver armor
            ...
            if analysis:
                ...
                if analysis.get('characters'):
                    messages.append(f"✓ Character detection working ({len(analysis['characters'])} found)")
                else:
                    messages.append("⚠ Character detection limited")
                
                # Check for other features
                if analysis.get('locations'):
                    messages.append(f"✓ Location detection working ({len(analysis['locations'])} found)")
                
                if analysis.get('themes'):
                    messages.append(f"✓ Theme detection working ({len(analysis['themes'])} found)")
            ...
            messages.append(f"✗ AI capability test failed: {str(e)}")
            return False, messages
    ...
    def _determine_content_category(self, file_path: Path, content: str) -> str:
        ...
        if any(keyword in path_lower for keyword in ['character', 'people', 'npc']):
            return 'characters'
        elif any(keyword in path_lower for keyword in ['location', 'place', 'setting']):
            return 'locations'
        elif any(keyword in path_lower for keyword in ['lore', 'history', 'background']):
            return 'lore'
        elif any(keyword in path_lower for keyword in ['story', 'narrative', 'scene']):
            return 'narrative'
    ...
    async def _enhance_category_analysis(self, base_analysis: Dict[str, Any], 
                                       category: str, content: str) -> Dict[str, Any]:
