...
- **Rate limiter ceiling.** `RateLimitMiddleware` defaults to 120 RPM per client IP (`OC_API_RATE_LIMIT_RPM`). Discovered 2026-05-11 during mnemosyne-mcp Phase C build: legitimate burst usage trips the limit. Mnemosyne's `mnemo_continue` makes 7 sequential `memory_search` calls just to gather context for one beat (one per entity type) — with a few back-to-back calls that adds up fast. The mnemosyne client now retries with exponential backoff (1s/2s/4s/8s/16s) which masks it in practice, but integration test suites that burst harder still trip occasionally. Two options worth considering: (a) raise the default to ~600 RPM (10 RPS) — single-user home-LAN deployments aren't IP-storming attackers; (b) expose a bulk-search endpoint (`memory_search_bulk(queries: list[SearchQuery])`) so multi-type retrievals are one HTTP call. Option (b) is the cleaner long-term fix; option (a) is a one-line config bump.
- **`project_delete` MCP tool / API surface.** OC v3 exposes `project_create` and `project_list` but no `project_delete`. Discovered 2026-05-11 during `mnemosyne-mcp` Phase A build: each integration test run creates a project (`mnemosyne-test-<timestamp>`) with marker memory, and there is no clean way to remove it short of manual SQLite. Workaround for now is to let test projects accumulate (they're identifiable by name prefix). The right fix is a `project_delete(project_id)` MCP tool plus `DELETE /api/v1/projects/{id}` REST endpoint. Cascade behavior needs an explicit choice: hard-delete the project AND all its memories, or refuse if memories exist. **Recommendation:** match `memory_delete`'s no-soft-delete posture — hard-delete with a `confirm: bool` flag (default `false` returns the memory count and refuses; `true` proceeds). Same project_id constraint applies as everywhere else (FK-rejected for unknown ids).
