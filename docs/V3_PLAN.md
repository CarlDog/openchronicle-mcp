...
12. **Cloud storage backup/sync for the memory store** — **DESIGNED 2026-08-23: see [design/0001-cloud-backup.md](design/0001-cloud-backup.md).** Backup-only is confirmed as the shape (sync-as-store stays in Out of Scope, and §9 of that doc verifies backup-only does not foreclose it). The design is *proposed*, not shipped: it awaits six operator decisions, chiefly provider choice and whether to encrypt. Original framing below, kept because the sub-questions are what the design answers:
