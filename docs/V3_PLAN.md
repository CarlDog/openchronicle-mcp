...
- **Codebase Remediation (Design 0013).** ✅ Completed 2026-09-18. Four phases addressing performance, scalability, concurrency, and portability:
  - **Phase 1:** Ollama probe retry cooldown (30.0s), empty-list SQL syntax guard in `list_embeddings`, parameter chunking for `WHERE id IN (...)` clauses.
  - **Phase 2:** SQL vector scope pushdown with `eligible_memory_ids` in `list_embeddings`, batched candidate hydration in `EmbeddingService` via `get_memories()`, persistent `httpx.Client` connection pooling for Ollama, decoupled rate limiter periodic sweep.
  - **Phase 3:** SQLite thread-local reader connections (`PRAGMA query_only = ON;`, `busy_timeout = 5000;`) unlocking non-blocking WAL reads, read-your-own-writes transaction consistency, and configurable background vector embedding (`background_embed=True`).
  - **Phase 4:** Standardized PEP 758 exception syntax to parenthesized tuples, broadened Python runtime compatibility floor to `>=3.12`, and enforced deterministic `uv.lock` consumption across Dockerfile and CI workflows (`uv sync --frozen`).
