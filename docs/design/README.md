...
| [0016 — Review findings plan](0016-review-findings-plan.md) | Plan and adversarial review | Track 1 query-revision fix and track 3 NAS Host-list fix entered `main` through PRs #34 and #35; no tag or deploy followed, and the detached live stack is unchanged. Track 2 has a bounded read-only live timestamp inventory showing 93 offset-bearing rows and active misordering; migration decisions and backup rehearsal remain gated. Prompt pilot and release decisions remain gated |
| [0017 — Exposed local backups and guarded restore preparation](0017-exposed-backup-and-restore.md) | Plan and adversarial review | Source implementation in `codex/exposed-backup-tools`; a 2026-09-24 Claude review's P1 findings are fixed on the PR (record in 0017). MCP tools parked while auth stays disabled. An off-NAS copy, the NAS mount and the disposable restore drill must precede the timestamp migration. The broader persistent-storage layout is a separate review item. |
