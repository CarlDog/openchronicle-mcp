...
| The operator can only reach the SMB share, which has no database or old backups. | *Superseded 2026-09-24: production already mounts the SMB-visible `config` folder at `/config`, so the runbook's Portainer-console route also works without a NAS shell or stack change; see the review disposition below.* Original text: This is a current bootstrap blocker. Obtain an approved NAS Docker admin/console path, run the existing v3.3.0 `oc db backup` against the live container, extract the resulting snapshot without raw-copying the live WAL database, then hash, open and retain it off-NAS. Do not change the stack first merely to expose the volume. Record the actual container/volume/path discovered at execution time. |
