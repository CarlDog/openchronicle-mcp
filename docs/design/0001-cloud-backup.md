...
>
> **Revision 6 (2026-08-23): the §11.3 watermark filter is shipped — §13.5 fully
> resolved.** `WATERMARK_SOURCE` is now filtered out of `export_memory` (stop
> producing the row) and `import_memory` in both `merge` and `replace` (stop
> importing a pre-fix envelope that still carries one — those are exactly the
> envelopes a first cross-device restore reads), each drop counted separately from
> collisions as `watermark_dropped`. This was the last OC-side prerequisite the
> design named; the only open operator question left is §13.4 (co-push the JSON
> envelope), and it is optional.
>
> **Revision 5 (2026-08-23): the `import --mode merge` hazard is fixed, not just
> flagged.** §11.4 is SHIPPED and §13.5 is half-resolved. Nothing about the backup
> feature itself changed — this was the prerequisite the design identified as "must
> be fixed before the first cross-device merge is attempted," and it is now the
> second such prerequisite landed (after `JobState.last_success_at`, §6.1). The
> remaining open questions are §13.4 (co-push the JSON envelope) and the §11.3
> watermark filter, which §13.5 still carries.
>
...
> being backed up, and Phase 0's gate decrypts the artifact once per identity.
>
...
**`config/core.json.example` is a third registration site.** **Revised 2026-08-28:** this section previously called it a live landmine, on the premise that `load_jobs` treated a present `maintenance.jobs` list as a total replacement. Since `5c884069` it **merges** config entries onto `_DEFAULT_JOBS` by name, so a job the example omits keeps its default instead of vanishing. Adding `cloud_backup` to the example is therefore documentation of the knob, not a correctness requirement — an operator who tunes one interval no longer loses cloud backup. The suggestion to extend `test_handlers_registry_complete` (which asserts `_DEFAULT_JOBS` == `HANDLERS`) to cover the example file loses its force for the same reason: under merge semantics the example cannot drop a job.
...
5. ~~**Fix the `oc memory import --mode merge` hazard (§11.4) and the watermark leak (§11.3) now, or park them?**~~ **RESOLVED 2026-08-23: both fixed.** The merge hazard shipped first (§11.4); the §11.3 watermark filter followed the same day — `export_memory` stops producing the row and `import_memory` drops it on read in both modes, so a pre-fix envelope restored to a fresh device can't resurrect it either. Nothing about this design remains blocked on an OC-side prerequisite; the only open operator question is §13.4 (co-push the JSON envelope), and it's optional.
