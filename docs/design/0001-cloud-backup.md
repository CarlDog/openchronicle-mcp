...
5. ~~**Fix the `oc memory import --mode merge` hazard (§11.4) and the watermark leak (§11.3) now, or park them?**~~ **HALF RESOLVED 2026-08-23: the merge hazard is fixed** (§11.4 SHIPPED). Neither blocked Phase 1 — cloud backup is push-only and nothing there performs a merge. But **cross-device restore is an explicit Goal of this design**, and the first restore-and-merge is the trigger condition, so this was never "operator's call, adjacent scope." **Still open: the §11.3 watermark filter** — ~2 lines, independently correct, and the merge fix does not cover it (a resurrected watermark is an insert, which merge is *supposed* to do; only the source filter stops it).
