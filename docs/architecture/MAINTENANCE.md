...
| `db_integrity_check` | 7 days | `PRAGMA integrity_check`. On failure: emergency `db_backup` (the snapshot then fails catalog verification too, so it is kept as `auto/*.db.failed-verify` rather than published), sets `container.maintenance_degraded = True` (surfaces via `/api/v1/health` and the MCP `health` tool), raises so the loop counts it. On success: clears any prior degraded flag. |
