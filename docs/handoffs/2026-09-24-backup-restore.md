...
current state (revs 217-226), [design 0017](../design/0017-exposed-backup-and-restore.md)
...
Their fixes are on draft PR #39 (revs 221-226). The operator decided on
...
3. Next source gate: an independent review of the fix round, then merge.
   Merging changes the nightly backup path, so the operator decides
   whether it rides v3.4.0.
