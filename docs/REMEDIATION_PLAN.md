...
- **Adversarial Plan Remediation:** Complete. Replaced raw background threads in `EmbeddingService` with bounded `ThreadPoolExecutor(max_workers=4)` and container lifecycle `close()`, tracked reader connections with dead-thread cleanup via `threading.enumerate()`, removed `@_locked` outer lock on `add_memories` to yield Python mutex between chunks with `UNIQUE` collision recovery, implemented direct SQL inner `JOIN` in `list_embeddings` scope pushdown, eliminated `unittest.mock` introspection from `OllamaEmbeddingAdapter`, converted `_client_lock` to `RLock` with double-checked probe synchronization, and normalized naive/aware datetimes in OCC `update_memory`. Tests: 1,067 passed, 1 skipped.
