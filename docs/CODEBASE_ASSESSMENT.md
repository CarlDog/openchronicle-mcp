...
**Snapshot date:** 2026-08-28 · **Revision:** 114
...
| Main vs deployed | **In sync.** The `v3.0.0` image is built from `4ba1c5d9`; every commit after it is documentation-only and skipped by `paths-ignore`, so none enters the image. Code goes live only when the stack's `OC_TAG` env moves; a push alone deploys nothing |
