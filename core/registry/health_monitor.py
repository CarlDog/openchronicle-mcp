...
class HealthMonitor:
    ...
    async def _check_adapter_health(self, adapter_name: str, adapter: Any) -> HealthCheckResult:
        ...
        try:
            ...
            elif response_time > self.health_config.get("unhealthy_response_threshold", 10.0):
                status = HealthStatus.UNHEALTHY
    ...
    async def _handle_health_change(self, result: HealthCheckResult):
        ...
        if (result.status == HealthStatus.DEGRADED and 
            self.health_config.get("alert_on_degraded", True)):
    ...
    async def _cleanup_old_results(self):
        ...
        for adapter_name in self.health_results:
            ...
            self.health_results[adapter_name] = [
                result for result in self.health_results[adapter_name]
    ...
    def get_performance_summary(self, adapter_name: str) -> Optional[PerformanceMetrics]:
    ...
    def get_overall_health_summary(self) -> Dict[str, Any]:
        ...
        healthy_count = sum(1 for status in self.current_status.values() 
                          if status == HealthStatus.HEALTHY)
        degraded_count = sum(1 for status in self.current_status.values() 
                           if status == HealthStatus.DEGRADED)
        unhealthy_count = sum(1 for status in self.current_status.values() 
        ...
        return {
            ...
            "last_check": max((r.timestamp for results in self.health_results.values() 
