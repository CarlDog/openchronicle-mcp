...
from core.shared.logging_system import log_system_event, log_info, log_warning
...
class BookmarkSearchEngine:
    ...
    def search_bookmarks(self, query: str, bookmark_type: Optional[BookmarkType] = None,
        ...
        try:
            ...
            log_system_event("bookmark_search", 
    ...
    def search_by_metadata(self, metadata_filters: Dict[str, Any],
                          bookmark_type: Optional[BookmarkType] = None) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            log_system_event("bookmark_metadata_search", 
    ...
    def search_by_scene_content(self, content_query: str,
                               bookmark_type: Optional[BookmarkType] = None) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            for row in rows:
                ...
                result['content_relevance'] = self._calculate_content_relevance(content_query, result)
                results.append(result)
            
            # Sort by content relevance
            results.sort(key=lambda x: x['content_relevance'], reverse=True)
            
            log_system_event("bookmark_content_search", 
    ...
    def find_similar_bookmarks(self, bookmark_id: int, similarity_threshold: float = 0.5) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            reference_row = self.execute_query(self.story_id, '''
            ...
            all_rows = self.execute_query(self.story_id, '''
                SELECT id, story_id, scene_id, label, description, bookmark_type, created_at, metadata
    ...
    def get_bookmark_suggestions(self, current_scene_id: str, context: Optional[str] = None) -> List[Dict[str, Any]]:
        """Get bookmark suggestions based on current context."""
        try:
            suggestions = []
            
            # Suggest based on recent bookmarks
            recent_bookmarks = self.execute_query(self.story_id, '''
            ...
            for bookmark in recent_bookmarks:
                if bookmark['scene_id'] != current_scene_id:  # Don't suggest current scene
                    suggestions.append({
                        'type': 'recent',
            ...
            if context:
                content_matches = self.search_by_scene_content(context[:100])  # Limit context length
    ...
    def _calculate_relevance(self, query: str, bookmark: Dict[str, Any]) -> float:
        ...
        label_matches = sum(1 for word in query_words if any(word in label_word for label_word in label_words))
        ...
        if bookmark['description']:
            ...
            desc_matches = sum(1 for word in query_words if any(word in desc_word for desc_word in desc_words))
            score += (desc_matches / len(query_words)) * 0.4
        
        return score
    
    def _calculate_content_relevance(self, query: str, bookmark: Dict[str, Any]) -> float:
    ...
    def _calculate_similarity(self, bookmark1: Dict[str, Any], bookmark2: Dict[str, Any]) -> float:
        ...
        if label1_words and label2_words:
            label_similarity = len(label1_words & label2_words) / len(label1_words | label2_words)
