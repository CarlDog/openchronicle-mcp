...
This module implements content-driven discovery and management of AI provider
configurations, replacing the monolithic model_registry.json approach with
individual, manageable configuration files.
...
class DynamicRegistryManager:
    ...
    def _validate_model_config(self, config: Dict[str, Any], config_file: Path) -> bool:
        ...
        """
        required_fields = ["provider", "display_name", "enabled", "adapter_class"]
        
        for field in required_fields:
            if field not in config:
                log_error(f"Config {config_file} missing required field: {field}")
                return False
    ...
    def get_model_config(self, config_name: str) -> Dict[str, Any]:
        ...
        Raises:
            ProviderNotFoundError: If configuration is not found
        """
        if config_name not in self.model_configs:
            raise ProviderNotFoundError(f"Model configuration '{config_name}' not found")
    ...
    def refresh_providers(self) -> Dict[str, List[Dict[str, Any]]]:
        """
        Refresh provider discovery (useful for detecting externally added configs).
        
        Returns:
            Updated providers dictionary
        """
    ...
    def get_registry_summary(self) -> Dict[str, Any]:
        ...
        try:
            ...
            for provider_models in self.providers.values():
                ...
                enabled_models += sum(1 for model in provider_models if model.get("enabled", False))
            
            return {
                "schema_version": self.global_settings.get("schema_version", "3.1.0"),
                ...
                "providers": {
                    provider: {
                        ...
                        "enabled_models": sum(1 for model in models if model.get("enabled", False))
                    }
            ...
            log_error(f"Failed to generate registry summary: {e}")
            return {"error": str(e)}
    
    def get_legacy_registry_format(self) -> Dict[str, Any]:
        """
        Generate a legacy registry format for backward compatibility.
        
        This method creates a registry structure compatible with the old
        monolithic model_registry.json format, allowing existing code to
        work without modification during the transition period.
        
        Returns:
            Dictionary in legacy registry format
        """
        try:
            # Start with global settings
            legacy_registry = dict(self.global_settings)
            
            # Add adapters section in legacy format
            adapters = {}
            ...
            for provider_name, models in self.providers.items():
                for model in models:
                    ...
                    # Use config_name as adapter key
                    adapter_key = model.get("config_name", f"{provider_name}_{model.get('display_name', 'model')}")
                    
                    # Convert to legacy adapter format
                    adapters[adapter_key] = {
                        "type": "text",  # Default type
                        "provider": provider_name,
                        "model": model.get("api_config", {}).get("model", ""),
                        "enabled": model.get("enabled", False),
                        "display_name": model.get("display_name", ""),
                        "adapter_class": model.get("adapter_class", ""),
                        **model.get("api_config", {}),
                        **model.get("capabilities", {}),
                        **model.get("limits", {})
                    }
            
            legacy_registry["adapters"] = adapters
            
            # Add fallback chains
            fallback_chains = {}
            for provider in self.providers.keys():
                fallback_chains[provider] = self.get_fallback_chain(provider)
            
            legacy_registry["fallback_chains"] = fallback_chains
            
            return legacy_registry
            
        except Exception as e:
            log_error(f"Failed to generate legacy registry format: {e}")
            return {}


# Convenience function for backward compatibility
def load_dynamic_registry(models_dir: str = "config/models/") -> DynamicRegistryManager:
    """
    ...
    Returns:
        Initialized DynamicRegistryManager instance
    """
    return DynamicRegistryManager(models_dir)
