...
from .mechanics_models import (
    MechanicsRequest, MechanicsResult, ResolutionResult, NarrativeBranch,
    ResolutionType, DifficultyLevel, OutcomeType, DiceRoll, ResolutionConfig,
...
class MechanicsOrchestrator:
    ...
    async def resolve_action(
        ...
        try:
            ...
            resolution_result.consequences, resolution_result.benefits = self._generate_consequences_and_benefits(
                resolution_result,
                character_state
            )
    ...
    async def simulate_action(
        ...
        try:
            ...
            if results:
                success_rate = sum(
                    1 for r in results 
                    if r.outcome in [OutcomeType.SUCCESS, OutcomeType.CRITICAL_SUCCESS, OutcomeType.PARTIAL_SUCCESS]
                ) / len(results)
    ...
    def _update_character_performance(
        ...
        if resolution_result.outcome in [OutcomeType.SUCCESS, OutcomeType.CRITICAL_SUCCESS, OutcomeType.PARTIAL_SUCCESS]:
            perf.successes += 1
        else:
            perf.failures += 1
    ...
    def _get_performance_summary(self, character_id: str) -> Dict[str, Any]:
        ...
        return {
            ...
            "recent_performance": len([r for r in perf.recent_rolls[-5:] 
