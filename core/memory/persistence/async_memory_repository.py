...
class AsyncMemoryRepository:
    ...
    async def load_memory(self, story_id: str) -> Dict[str, Any]:
        ...
        try:
            ...
            rows = await self.db_orchestrator.execute_query(story_id, '''
                SELECT type, key, value FROM memory 
                WHERE story_id = ? AND key = "current"
                ORDER BY updated_at DESC
            ''', (story_id,), is_test=False)
    ...
    async def save_memory(self, story_id: str, memory: Dict[str, Any]) -> bool:
        ...
        try:
            ...
            for memory_type, json_data in operations:
                task = self.db_orchestrator.execute_update(story_id, '''
                    INSERT OR REPLACE INTO memory (story_id, type, key, value, updated_at)
                    VALUES (?, ?, "current", ?, ?)
                ''', (story_id, memory_type, json_data, timestamp), is_test=False)
                tasks.append(task)
            ...
            if success:
                ...
                logger.error(f"Failed to save memory for story {story_id}: some operations failed")
    ...
    def get_character_memory_cached(self, story_id: str, character_name: str) -> Optional[Dict[str, Any]]:
    ...
    async def load_character_memory(self, story_id: str, character_name: str) -> Optional[Dict[str, Any]]:
            ...
            try:
                ...
                for key, value in updates.items():
                    if key in character_data:
                        if isinstance(character_data[key], dict) and isinstance(value, dict):
    ...
    async def save_world_state(self, story_id: str, world_state: Dict[str, Any]) -> bool:
        ...
        try:
            ...
            success = await self.db_orchestrator.execute_update(story_id, '''
                INSERT OR REPLACE INTO memory (story_id, type, key, value, updated_at)
                VALUES (?, "world_state", "current", ?, ?)
            ''', (story_id, json_data, timestamp), is_test=False)
    ...
    async def create_memory_snapshot(self, story_id: str, scene_id: str) -> bool:
        ...
        try:
            ...
            success = await self.db_orchestrator.execute_update(story_id, '''
                ...
                VALUES (?, ?, ?, ?)
            ''', (story_id, scene_id, json_data, timestamp), is_test=False)
    ...
    async def restore_memory_snapshot(self, story_id: str, scene_id: str) -> bool:
        ...
        try:
            ...
            ''', (story_id, scene_id), is_test=False)
