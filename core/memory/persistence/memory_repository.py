...
from ..shared.memory_models import MemoryState, MemorySnapshot, MemoryType, MemoryUpdateResult
...
class MemoryRepository:
    ...
    def restore_from_snapshot(self, story_id: str, scene_id: str) -> Optional[MemoryState]:
    ...
    def _serialize_memory(self, memory: MemoryState) -> Dict[str, Any]:
        ...
        return {
            ...
            "flags": [{"name": flag.name, "created": flag.created.isoformat(), "data": flag.data} 
                     for flag in memory.flags],
            "recent_events": [{"description": event.description, "timestamp": event.timestamp.isoformat(), 
                             "data": event.data} for event in memory.recent_events],
            "metadata": {
    ...
    def _deserialize_memory(self, data: Dict[str, Any]) -> MemoryState:
        ...
        from ..shared.memory_models import MemoryState, CharacterMemory, MemoryFlag, RecentEvent, MemoryMetadata, MoodEntry, VoiceProfile
