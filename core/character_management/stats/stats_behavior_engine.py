...
from ..character_base import CharacterEngineBase, CharacterBehaviorProvider, CharacterValidationProvider
...
class StatsBehaviorEngine(CharacterEngineBase, CharacterBehaviorProvider, CharacterValidationProvider):
    ...
    def __init__(self, config: Optional[Dict] = None):
        ...
        self.stat_interactions: Dict[str, List[Tuple[CharacterStatType, CharacterStatType]]] = self._initialize_stat_interactions()
    ...
    def add_temporary_stat_modifier(self, character_id: str, stat_type: CharacterStatType,
                                  modifier: int, duration_hours: int = 24) -> bool:
    ...
    def get_consistency_score(self, character_id: str) -> float:
        ...
        for progression in recent_progressions:
            ...
            if stat_value >= 9 and len([p for p in stats.progression_history 
                                      if p.stat_type == progression.stat_type]) < 3:
    ...
    def import_character_data(self, character_data: Dict[str, Any]) -> None:
        ...
        if character_id and stats_data:
            try:
                ...
                self.logger.error(f"Failed to import stats data for {character_id}: {e}")
    ...
    def _initialize_stat_interactions(self) -> Dict[str, List[Tuple[CharacterStatType, CharacterStatType]]]:
    ...
    def _get_stat_influences(self, stat_type: CharacterStatType, value: int, situation: str) -> List[CharacterBehaviorInfluence]:
    ...
    def _validate_stat_update(self, stats: CharacterStats, action: Dict[str, Any]) -> Tuple[bool, Optional[str]]:
    ...
    def _validate_decision(self, stats: CharacterStats, action: Dict[str, Any]) -> Tuple[bool, Optional[str]]:
        ...
        for stat_name, min_value in required_stats.items():
            try:
                ...
                if current_value < min_value:
                    return False, f"Decision requires {stat_name} >= {min_value}, character has {current_value}"
            except ValueError:
                continue
        
        return True, None
    
    def _validate_behavior(self, stats: CharacterStats, action: Dict[str, Any]) -> Tuple[bool, Optional[str]]:
        ...
        for limitation in limitations:
            if behavior_type in limitation:
                return False, f"Behavior conflicts with character limitation: {limitation}"
