...
class StoryGenerator:
    ...
    def generate_coherent_narrative(
        ...
        participant_characters: List['Character']
    ) -> ValidationResult:
...
class CharacterAnalyzer:
    ...
    def validate_character_concept(
        ...
        if not personality_traits:
            ...
            for trait1, trait2 in trait_conflicts:
                if (trait1 in personality_traits and trait2 in personality_traits and
                    personality_traits[trait1] > 7 and personality_traits[trait2] > 7):
        ...
        if story_context:
            ...
            if "medieval" in setting and any(word in background.lower() for word in ["computer", "internet", "smartphone"]):
    ...
    def _generate_character_suggestions(
        ...
        if personality_traits:
            high_traits = [trait for trait, value in personality_traits.items() if isinstance(value, (int, float)) and value > 8]
            if high_traits:
                suggestions.append(f"Consider how extreme {', '.join(high_traits)} traits affect relationships")
        
        return suggestions
    
    def calculate_consistency_score(self, character: Character, 
                                  recent_scenes: List[Scene]) -> float:
    ...
    def suggest_character_development(self, character: Character, 
                                    story_context: NarrativeContext) -> List[str]:
        """Suggest potential character development opportunities."""
        suggestions = []
        
        # Check if character has been static for too long
        recent_development = [
            event for event in character.character_arc
    ...
    def validate_character_update(self, character: Character, 
                                proposed_updates: Dict[str, Any]) -> Tuple[bool, List[str]]:
    ...
    def analyze_consistency(self, character: Character, scene_content: str, 
                          previous_scenes: List[Scene]) -> 'ConsistencyAnalysisResult':
        ...
        if char_name_lower in scene_lower:
            ...
            emotional_words = {
                "angry": 0.8, "furious": 0.9, "calm": 0.2, "peaceful": 0.1,
                "sad": 0.7, "happy": 0.3, "excited": 0.8, "worried": 0.6
            }
...
class CharacterService(ABC):
    ...
    @abstractmethod
    async def get_character(self, story_id: str, character_id: str) -> Optional[Character]:
        """Get character by ID."""
        pass
