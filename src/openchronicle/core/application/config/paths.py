...
Four-layer precedence: constructor param > per-path env var > OC_DATA_DIR-derived > hardcoded default.
"""
...
# ── Default constants (match historical hardcoded values) ────────────────
...
DEFAULT_DISCORD_SESSION_PATH = "data/discord_sessions.json"
DEFAULT_DISCORD_PID_PATH = "data/discord_bot.pid"
...
class RuntimePaths:
    """Resolved runtime paths for all OC data artifacts."""
    ...
    output_dir: Path
    assets_dir: Path
    discord_session_path: Path
    discord_pid_path: Path
    ...
    def resolve(
        ...
        *,
        ...
        discord_session_path: str | Path | None = None,
        discord_pid_path: str | Path | None = None,
    ) -> RuntimePaths:
        """Build RuntimePaths with four-layer precedence.

        Constructor params > per-path env vars > OC_DATA_DIR-derived > defaults.
        """
        ...
        return cls(
            ...
            plugin_dir=_resolve(plugin_dir, "OC_PLUGIN_DIR", data_dir, "plugins", DEFAULT_PLUGIN_DIR),
            output_dir=_resolve(output_dir, "OC_OUTPUT_DIR", data_dir, "output", DEFAULT_OUTPUT_DIR),
            assets_dir=_resolve(assets_dir, "OC_ASSETS_DIR", data_dir, "assets", DEFAULT_ASSETS_DIR),
            discord_session_path=_resolve(
