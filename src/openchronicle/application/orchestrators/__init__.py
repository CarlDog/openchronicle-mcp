...
from ..commands import (
    Command, CommandResult, CreateStoryCommand, UpdateStoryCommand,
    CreateCharacterCommand, UpdateCharacterCommand, GenerateSceneCommand,
    SaveSceneCommand, UpdateMemoryCommand, RollbackStoryCommand
)
from ..queries import Query, QueryResult
from ...domain import (
    Story, Character, Scene, StoryGenerator, CharacterAnalyzer,
    MemoryState, NarrativeContext, ModelResponse
)
...
class ModelManager(Protocol):
    ...
    async def generate_response(self, context: NarrativeContext, model_preference: Optional[str] = None) -> ModelResponse:
        ...
        try:
            ...
            if not validation_result.is_valid:
                return CommandResult.failure(
                    "Story concept validation failed",
            ...
            await self.memory_manager.update_memory(story.id, {
                "world_state": command.initial_world_state,
                "characters": {},
                "events": [],
                "flags": {}
        ...
        try:
            ...
            if command.world_state_updates:
                await self.memory_manager.update_memory(
                    story.id, 
                    {"world_state": command.world_state_updates}
                )
...
class CharacterOrchestrator(BaseOrchestrator):
    ...
    async def create_character(self, command: CreateCharacterCommand) -> CommandResult:
        ...
        try:
            ...
            # Update memory
            await self.memory_manager.update_memory(command.story_id, {
                "characters": {
                    character.id: {
                        "name": character.name,
    ...
    async def generate_scene(self, command: GenerateSceneCommand) -> CommandResult:
        ...
        try:
            ...
            if not narrative_result.is_valid:
                return CommandResult.failure(
                    "Narrative generation validation failed",
                    narrative_result.errors
                )
            ...
            return CommandResult.success("Scene generated successfully", {
