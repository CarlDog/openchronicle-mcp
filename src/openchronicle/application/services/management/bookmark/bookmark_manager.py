...
class BookmarkManager:
    ...
    def create_bookmark(
        ...
        validation = BookmarkValidator.validate_bookmark_data(
            scene_id, label, bookmark_type, metadata
        )
        if not validation["is_valid"]:
            raise BookmarkManagerException(
                f"Invalid bookmark data: {', '.join(validation['errors'])}"
            )

        return self.data_manager.create_bookmark(
    ...
    def update_bookmark(
        ...
        return self.data_manager.update_bookmark(
            bookmark_id, label, description, bookmark_type, metadata
        )
    ...
    def search_bookmarks(
        ...
        return self.search_engine.search_bookmarks(
            query, bookmark_type, search_fields, limit
        )
    ...
    def auto_create_chapter_bookmark(
        self, scene_id: str, chapter_title: str, chapter_level: int = 1
    ) -> int:
        ...
        try:
            ...
            for bookmark in all_bookmarks:
                ...
                try:
                    ...
                    issues.append(
                        f"Bookmark {bookmark['id']} has invalid type: {bookmark['bookmark_type']}"
                    )
        ...
        try:
            ...
            for update in updates:
                ...
                try:
                    ...
                    results["failed"] += 1
                    results["errors"].append(
                        f"Error updating bookmark {bookmark_id}: {e}"
                    )
