...
class InputValidator:
    ...
    def validate_user_input(
        ...
        if len(content) > self.max_input_length:
            log_warning(
                f"Input length exceeded: {len(content)} > {self.max_input_length}",
        ...
        for pattern in self.DANGEROUS_SCRIPT_PATTERNS:
            if re.search(pattern, content, re.IGNORECASE):
                log_critical(
                    f"Script injection attempt detected: {pattern}",
                    context_tags={
                        "security": "script_injection",
    ...
    def validate_file_path(
        ...
        try:
            ...
            if not any(normalized_path.is_relative_to(root) for root in allowed_roots):
                log_warning(
                    f"File access outside allowed directories: {normalized_path}",
    ...
    def validate_json_data(
        ...
        try:
            ...
            if danger_result:
                log_critical(
                    f"Dangerous JSON content detected: {danger_result}",
                    context_tags={
                        "security": "json_validation",
                        "user": context.user_id,
                    },
                )
                return SecurityValidationResult(
...
class FileAccessManager:
    ...
    def safe_read_file(
        ...
        if not validation_result.is_valid:
            log_warning(
                f"File read denied: {validation_result.error_message}",
                context_tags={"security": "file_access", "user": context.user_id},
            )
    ...
    def safe_write_file(
        ...
        if not validation_result.is_valid:
            log_warning(
                f"File write denied: {validation_result.error_message}",
                context_tags={"security": "file_access", "user": context.user_id},
            )
            return False, validation_result.error_message
        ...
        if not content_validation.is_valid:
            log_warning(
                f"File content validation failed: {content_validation.error_message}",
                context_tags={
                    "security": "content_validation",
                    "user": context.user_id,
                },
            )
...
class SecurityMonitor:
    ...
    def record_security_violation(
        ...
        if threat_level == SecurityThreatLevel.CRITICAL:
            log_critical(
                f"CRITICAL SECURITY VIOLATION: {violation_type.value} - {details}",
                context_tags={
                    "security": "critical_violation",
                    "user": context.user_id,
                },
            )
...
class SecurityManager:
    ...
    def validate_and_sanitize(
        self, data: Any, validation_type: str, context: SecurityContext
    ) -> SecurityValidationResult:
