...
class CentralizedConfigManager:
    ...
    def _ensure_storage_directories(self):
        ...
        if created_paths:
            log_info(
                f"Auto-created {len(created_paths)} missing storage directories",
    ...
    def get_config_value(self, section: str, key: str) -> Any:
        ...
        try:
            ...
            log_warning(
                f"Unknown config path: {section}.{key}",
                context_tags=["config", "warning"],
            )
            return None
    ...
    def validate_config(self) -> list[str]:
        ...
        if not issues:
            ...
            log_warning(
                f"Configuration validation found {len(issues)} issues",
