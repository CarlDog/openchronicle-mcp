...
class TestStoryCreationWorkflow:
    ...
    @pytest.mark.asyncio
    async def test_complete_story_creation_workflow(self, test_story_data, workflow_orchestrators):
        ...
        opening_scene_data = {
            ...
            "active_characters": [char["name"] for char in test_story_data["initial_characters"]],
            "scene_type": "opening",
            "timestamp": time.time()
        }
        ...
        content_analysis = await orchestrators["content"].analyze_content(
            ...
            context={
                ...
                "characters": [char["name"] for char in test_story_data["initial_characters"]]
            }
        )
        ...
        await orchestrators["memory"].add_recent_event(
            story_id=story_id,
            event_data=scene_memory_update
        )
        ...
        story_context = await orchestrators["context"].build_character_focused_context(
            ...
            character_name=test_story_data["initial_characters"][0]["name"]  # Focus on first character
        )
        ...
        assert len(memory_summary.get("active_flags", [])) >= 0  # May have flags from character creation
        assert memory_summary.get("character_count", 0) >= len(test_story_data["initial_characters"])
        ...
        workflow_metrics = {
            ...
            "scenes_created": 1,
            "memory_updates": len(test_story_data["initial_characters"]) + 1,  # Character updates + scene update
        ...
        print(f"🏁 Story creation workflow completed successfully!")
    ...
    @pytest.mark.asyncio
    async def test_story_creation_error_recovery(self, test_story_data, workflow_orchestrators):
        ...
        with pytest.raises(Exception):
            await orchestrators["scene"].create_scene(
                ...
                scene_data={"invalid": "data"}
            )
        
        print("✅ Error recovery tests passed")
        
    @pytest.mark.asyncio
    async def test_concurrent_story_creation(self, test_story_data, workflow_orchestrators):
