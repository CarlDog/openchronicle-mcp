...
from core.model_management.model_interfaces import (
...
from core.model_management.model_orchestrator import (
    ModelResponseGenerator, ModelLifecycleManager,
    ModelOrchestrator
)

from core.memory_management.memory_interfaces import (
...
class TestModelInterfaceSegregation:
    ...
    async def test_response_generator_successful_generation(self):
        ...
        self.mock_config_manager.get_model_configuration.return_value = ModelConfiguration(
...
class TestInterfaceSegregationBenefits:
    ...
    def test_interface_focused_testing(self):
        ...
        assert not hasattr(mock_generator, 'initialize_adapter')  # Not part of this interface
        ...
        mock_generator.generate_response.assert_called_once_with("test_prompt", "test_adapter")
    ...
    def test_interface_substitutability(self):
        ...
        class MockResponseGenerator(IModelResponseGenerator):
            async def generate_response(self, prompt, adapter_name, model_params=None, use_fallback=True):
            ...
            async def generate_with_fallback_chain(self, prompt, adapter_chain, model_params=None):
                return await self.generate_response(prompt, adapter_chain[0], model_params)
    ...
    def test_dependency_injection_compatibility(self):
        ...
        class TestResponseGenerator:
            ...
            async def generate_response(self, prompt: str, adapter_name: str = None) -> dict:
        ...
        container.register(IModelResponseGenerator, TestResponseGenerator, ServiceLifetime.SINGLETON)
        container.register(IModelLifecycleManager, TestLifecycleManager, ServiceLifetime.SINGLETON)
    ...
    def test_interface_single_responsibility(self):
        ...
        response_methods = [method for method in dir(IModelResponseGenerator) 
                          if not method.startswith('_')]
        assert all('response' in method.lower() or 'fallback' in method.lower() 
                  for method in response_methods)
        
        # Lifecycle management interface should only handle lifecycle
        lifecycle_methods = [method for method in dir(IModelLifecycleManager) 
                           if not method.startswith('_')]
        lifecycle_keywords = ['initialize', 'health', 'shutdown', 'restart', 'available', 'healthy']
        assert all(any(keyword in method.lower() for keyword in lifecycle_keywords) 
