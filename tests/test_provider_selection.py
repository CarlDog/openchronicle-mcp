...
async def test_openai_provider_without_key_fails(tmp_path: Path, monkeypatch: Any) -> None:
    """With OC_LLM_PROVIDER=openai but invalid API key, call fails at execution time."""
    monkeypatch.setenv("OC_LLM_PROVIDER", "openai")
    # Set invalid API key to simulate misconfiguration
    monkeypatch.setenv("OPENAI_API_KEY", "changeme-invalid-key")

    # Container init succeeds - provider is configured but key is invalid
    ...
    # Should fail at execution time with authentication error (401)
    with pytest.raises(LLMProviderError, match="Error code: 401"):
